...
class Sat2LidarDataset(GeneralizedDataset):
    ...
    def get_image(self, img_id):
        ...
        image = np.pad(image, ((0, self.target_hw - image.shape[0]) , (0, self.target_hw - image.shape[1]), (0, 0)), mode='constant', constant_values=(0))
        if image.shape[-1] == 4:
            image = image[..., :3]
    ...
    '''def load_lidar(self, image_id, normalized=True):
        ann = self.get_annotation(image_id)
        lidar = ann["lidar"]
        lidar = np.array(lidar, 'float32')
        if normalized == True:
    ...
    def rotate_lidar(self, lidar, rotation):
        median is not correct
        theta = np.deg2rad(rotation * 90)
        
        rot_mat = np.asarray([[np.cos(theta), -np.sin(theta), 0], [np.sin(theta), np.cos(theta), 0], [0, 0, 1]])
        lidar -= [np.median(lidar[:, 0]), np.median(lidar[:, 1]), 0]
        lidar = np.transpose(lidar)
        lidar = np.matmul(rot_mat, lidar)
        lidar = np.transpose(lidar)
        lidar += [np.median(lidar[:, 0]), np.median(lidar[:, 1]), 0]
    ...
    def __getitem__(self, i):
        ...
        image = self.data_augmentation(image, rotation, True)
        image = (np.transpose(image / 255.0, (2, 0, 1)) - .5) * 2

        #grey_image = transforms.ToTensor()(grey_image)
        

        target = self.get_target(img_id, rotation)

        r_vec = self.get_init_pc() #todo change this to random vector
        return r_vec, image, target   
